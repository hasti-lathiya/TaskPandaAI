...
function StreakRepairCard({ userStats }) {
  ...
  return (
    <div className="bg-white/40 dark:bg-[#0f172a]/40 backdrop-blur-xl border border-amber-300/60 dark:border-amber-500/30 rounded-[32px] p-6 sm:p-8 mb-8 relative overflow-hidden shadow-sm shadow-amber-500/5">
      ...
      <div className="relative flex flex-col lg:flex-row lg:items-center gap-6 justify-between">
        <div className="flex items-start gap-4">
          <div className="w-14 h-14 shrink-0 rounded-2xl bg-amber-500/10 dark:bg-amber-500/20 flex items-center justify-center border border-amber-400/20">
            <Flame className="text-amber-500" size={28} />
          </div>

          <div>
            <h3 className="text-xl font-black text-slate-800 dark:text-slate-50 tracking-tight flex items-center gap-2">
              Your {repairable.streak}-day streak broke
            </h3>
            <p className="text-slate-600 dark:text-slate-400 text-sm font-medium mt-1 max-w-md">
        ...
        <div className="flex items-center gap-4 shrink-0">
          <div className="text-right">
            <div className="flex items-center gap-1.5 text-2xl font-black text-slate-800 dark:text-slate-50">
          ...
          <button
            ...
            className="bg-amber-500 hover:bg-amber-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold text-sm px-6 py-3.5 rounded-xl transition cursor-pointer whitespace-nowrap"
          >
            {busy ? "Restoring..." : affordable ? "Restore streak" : "Not enough coins"}
          </button>
        </div>
      </div>
    </div>
