    ...
    <header className="relative bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl px-3.5 sm:px-6 py-3 sm:py-4 flex items-center justify-between w-full shadow-sm transition-colors duration-300 mb-6">
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div ref={dropdownRef} className="relative">
          ...
          {/* Dropdown Panel */}
          {notificationsOpen && (
            <div className="fixed inset-x-3 top-20 sm:absolute sm:inset-x-auto sm:right-0 sm:top-full mt-3 sm:w-96 w-auto max-w-[calc(100vw-1.5rem)] sm:max-w-none bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl shadow-2xl z-50 p-4 text-slate-800 dark:text-white animate-in fade-in slide-in-from-top-3 duration-200">
              ...
              <div className="flex items-center justify-between pb-3 mb-3 border-b border-gray-100 dark:border-slate-800">
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 text-xs px-2.5 py-0.5 rounded-full font-bold">
                    {unreadCount} Unread
                  </span>
                </div>
                <button
                  ...
                  className="text-slate-400 hover:text-slate-700 dark:hover:text-white p-1 rounded-lg transition text-lg leading-none cursor-pointer"
                >
                  ✕
                </button>
              </div>
              ...
              {notifications.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-xs text-gray-500 dark:text-slate-400 mt-1 max-w-[200px]">
                  ...
                  {notifications.map((n) => (
                    <div
                      ...
                      className={`p-3 rounded-xl transition border cursor-pointer ${
                        n.read
                          ? "bg-slate-50/70 dark:bg-slate-800/40 border-gray-100 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800/70"
                          : "bg-indigo-50/40 dark:bg-slate-800/80 border-indigo-100 dark:border-indigo-900/40 hover:bg-indigo-50/70 dark:hover:bg-slate-800"
                      }`}
                    >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between items-start gap-1">
                            <h4 className={`text-sm truncate ${
                              n.read ? "text-slate-500 dark:text-slate-400 font-medium" : "text-slate-900 dark:text-slate-100 font-bold"
                            }`}>
                              {n.title}
                            </h4>
                            {!n.read && (
                              <span className="w-2 h-2 rounded-full bg-indigo-500 dark:bg-indigo-400 mt-1.5 flex-shrink-0" />
                            )}
                          </div>
                          <p className="text-xs text-slate-600 dark:text-slate-300 mt-1 leading-relaxed break-words font-medium">
                            {n.message}
                          </p>
                          <span className="text-[10px] text-indigo-600 dark:text-indigo-400 font-semibold mt-2 block">
              ...
              {notifications.length > 0 && notifications[0].id !== "welcome-notif" && (
                <div className="flex justify-between gap-3 pt-3 border-t border-gray-100 dark:border-slate-800 text-xs">
                  <button
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-red-500 dark:text-red-400 hover:underline font-bold cursor-pointer"
                  >
